...
const getDHMDisplacement = ({ time, startTime, mass, stiffness, damping, initialDisplacement, initialVelocity }) => {
  ...
  return { shouldStop, displacement: shouldStop ? 0 : displacement };
};
...
const DSMS = () => {
  ...
  useEffect(() => {
    ...
    const tick = time => {
      ...
      const { displacement, shouldStop } = getDHMDisplacement({ time, startTime, mass, stiffness, damping, initialDisplacement, initialVelocity });
      if (shouldStop) stop();
      else {
